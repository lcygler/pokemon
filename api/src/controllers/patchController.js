...
const patchUpdate = async (idPokemon, pokemon) => {
  ...
  //* Check name
  if (name) {
    const findName = await Pokemon.findOne({
      where: {
        name: pokemonName,
      },
    });
  ...
  //* Update pokemon
  const pokemonUpdate = {
    name: name ? pokemonName : dbPokemon.name,
    hp: hp ? hp : dbPokemon.hp,
    attack: attack ? attack : dbPokemon.attack,
    defense: defense ? defense : dbPokemon.defense,
    speed: speed ? speed : dbPokemon.speed,
    height: height ? height : dbPokemon.height,
    weight: weight ? weight : dbPokemon.weight,
    image: image ? image : dbPokemon.image,
  };
